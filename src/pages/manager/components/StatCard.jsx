            ...
            <AreaChart data={chartData}>
              <defs>
                <linearGradient id={`gradient-${title}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={color.includes('emerald') ? '#10b981' : color.includes('primary') ? '#8B5CF6' : color.includes('amber') ? '#F59E0B' : '#8B5CF6'} stopOpacity={0.3}/>
                  <stop offset="95%" stopColor={color.includes('emerald') ? '#10b981' : color.includes('primary') ? '#8B5CF6' : color.includes('amber') ? '#F59E0B' : '#8B5CF6'} stopOpacity={0}/>
                </linearGradient>
              </defs>
              <Area
                type="monotone"
                dataKey="value"
                stroke={color.includes('emerald') ? '#10b981' : color.includes('primary') ? '#8B5CF6' : color.includes('amber') ? '#F59E0B' : '#8B5CF6'}
                strokeWidth={2}
