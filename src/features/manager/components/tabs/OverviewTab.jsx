    ...
    <div className="space-y-6 md:space-y-8 animate-fade-in">
      ...
      <div className="md:hidden -mx-4 px-4 overflow-x-auto scrollbar-hide pb-2">
        <div className="flex gap-2 min-w-max">
          <QuickAction icon={ShoppingCart} label="Orders" onClick={() => onTabChange('orders')} color="text-primary" />
          <QuickAction icon={LayoutGrid} label="Tables" onClick={() => onTabChange('tables')} color="text-accent" />
          <QuickAction icon={ChefHat} label="Kitchen" onClick={() => onTabChange('kitchen')} color="text-emerald-400" />
          <QuickAction icon={Users} label="Staff" onClick={() => onTabChange('staff')} color="text-blue-400" />
          <QuickAction icon={FileText} label="Reports" onClick={() => navigate('/manager/reports')} color="text-purple-400" />
        </div>
      </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg border shadow-lg ${
                          order.order_status === 'completed' ? 'bg-zinc-500/10 border-zinc-500/20 text-zinc-500' :
                          'bg-white/5 border-white/10 text-zinc-400'
                        }`}>
                          <ShoppingBag className="h-4 w-4" />
                        </div>
                        <div>
                          <h4 className="text-sm font-bold text-white font-mono-nums">#{order.order_number}</h4>
