...
const OrdersTab = () => {
  ...
  useEffect(() => {
    ...
    if (
      initialFilter &&
      ['all', 'active', 'pending', 'completed', 'cancelled'].includes(initialFilter)
    ) {
      ...
      <div className="glass-panel p-4 rounded-2xl border border-white/10 space-y-4 flex-shrink-0">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            ...
            >
              ...
              Pending Payments
              {pendingCount > 0 && (
                <span className="ml-2 px-2 py-0.5 text-xs rounded bg-amber-500/20 border border-amber-500/40">
                  {pendingCount}
                </span>
              )}
            </button>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2 border-t border-white/5">
          ...
          <div className="flex items-center gap-4 text-xs font-medium text-zinc-400 bg-black/20 px-3 py-1.5 rounded-lg border border-white/5">
            <span>
              Orders: <span className="text-white font-mono">{viewStats.count}</span>
            </span>
            <span className="w-px h-3 bg-white/10" />
            <span>
              Revenue:{' '}
              <span className="text-emerald-400 font-mono">
                {formatCurrency(viewStats.revenue)}
              </span>
            </span>
          </div>
        </div>
        ...
        onCreate={async (rawData) => {
          try {
            ...
            const { generateOrderNumber, generateOrderToken, calculateTax, calculateTotal } =
              await import('@features/orders/utils/orderHelpers.js');
