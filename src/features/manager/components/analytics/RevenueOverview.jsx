...
const RevenueOverview = ({ data = [], loading = false }) => {
      ...
      return (
        <div className="bg-white dark:bg-card p-3 rounded-lg shadow-lg border border-gray-200 dark:border-border">
          ...
          {payload.map((entry, index) => (
            <p key={index} className="text-sm text-gray-600 dark:text-muted-foreground">
  ...
  return (
    <div className="bg-white dark:bg-card rounded-lg p-6 border border-gray-200 dark:border-border shadow-sm">
      ...
      {/* Summary Stats */}
      <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-gray-200 dark:border-border">
        <div className="text-center">
          <p className="text-xs text-gray-600 dark:text-muted-foreground mb-1">Avg Monthly</p>
          <p className="text-sm font-semibold text-gray-900 dark:text-foreground">
            {formatCurrency(data.length > 0 ? totalRevenue / data.length : 0)}
          </p>
        </div>
        <div className="text-center">
          <p className="text-xs text-gray-600 dark:text-muted-foreground mb-1">Peak Month</p>
          <p className="text-sm font-semibold text-gray-900 dark:text-foreground">
            {data.length > 0 
        ...
        <div className="text-center">
          <p className="text-xs text-gray-600 dark:text-muted-foreground mb-1">Growth</p>
